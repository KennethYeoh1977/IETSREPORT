...
def save_report_as_docx(report_text, file_name, directory):
    ...
    # Save the document as .docx in the specified directory
    file_path = os.path.join(directory, file_name)
    doc.save(file_path)

def main():
    st.title("Wastewater Treatment / IETS  Data +Report Generator")
    ...
    if uploaded_file is not None:
        ...
        # User input for directory and file name
        directory = st.text_input("Enter the  directory (c|user|download|) to save the report :", value=".")
        file_name = st.text_input("Enter the file name for the report i.e IETS_treatment_report_Jan202X.docx: ", value="IETS_treatment_report_.docx")

        # Button to save report as .docx
        if st.button("Save Report as .docx"):
            if directory and file_name:
                save_report_as_docx(report_text, file_name, directory)
                st.success(f"Report saved as {os.path.join(directory, file_name)}")
            else:
                st.error("Please provide both a directory and a file name.")
